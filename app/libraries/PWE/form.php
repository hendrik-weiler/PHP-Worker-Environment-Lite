<?php
/**
 * form class - generates a complete form
 * (supports method chaining)
 *
 *
 * @author Hendrik Weiler
 */
class form extends cases {
    private $form_open;
    private $text;
    private $button;
    private $close;
    ...
    {
        if(empty($action)) { 
              throw new unError(message::$parameter, "open");
        }  
        if(is_array($attr))
        {
        ...
        {
           $action = url::validate($action, true);
        }
    ...
    {
        if(empty($file)) { 
              throw new unError(message::$parameter, "addhtml");
        }         
        if(file_exists($file))
        {
     ...
     * @return form 
     */
    function tfield($name,$value,$attr=null)
    {
        if(empty($name)) { 
              throw new unError(message::$parameter, "tfield");
        }         
        if(is_array($attr))
        {
     ...
     * @return form
     */
    function label($value,$mode=0)
    {
        if(empty($value)) { 
              throw new unError(message::$parameter, "label");
        } 
        if($mode == 0)
        {
            $this->text[] = "$value:";
    ...
    function listbox($name,$array,$attr=null)
    {
        if(empty($name) || empty($array)) { 
              throw new unError(message::$parameter, "listbox");
        } 
    ...
    {
        if(empty($name)) { 
              throw new unError(message::$parameter, "hidden");
        }         
        if(is_array($attr))
        {
            foreach($attr as $key => $val)
    ...
    function checkbox($name,$value,$attr=null)
    {
        if(empty($name) || empty($value)) { 
              throw new unError(message::$parameter, "checkbox");
        } 
        if(is_array($attr))
        {
            foreach($attr as $key => $val)
    ...
    {
        if(empty($name)) { 
              throw new unError(message::$parameter, "tarea");
        }         
        if(is_array($attr))
        {
            foreach($attr as $key => $val)
    ...
    {
        if(empty($value)) { 
              throw new unError(message::$parameter, "button");
        } 
        if(is_array($attr))
        {
            foreach($attr as $key => $val)
